...
class PlayerManager:
    ...
    def write_output(self, text, player):
        ...
        tooltip_status = (
            f"<span color='#f38ba8'><b>Paused:</b> </span>"
    ...
    def on_metadata_changed(self, player, metadata, _=None):
        ...
        if metadata:
            try:
                ...
                track_id_variant = metadata.lookup_value(
                    "mpris:trackid", GLib.VariantType("s")
                )
        ...
        if track_info:
            prefix = (
                f"<span color='#a6e3a1'>󰓇  </span>"
                if player.props.status == "Playing" and player_name == "spotify"
                else f"<span color='#f38ba8'>󰗃  </span>"
                if player.props.status == "Playing" and player_name == "firefox"
                else f"<span color='#74c7ec'>󰼁  </span>"
                if player.props.status == "Playing" and player_name == "jellyfin-tui"
                else f"<span color='#b4befe'>\u200a󰏤 \u2009\u2009\u200a</span>"
            )
